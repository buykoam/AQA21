...
public class CssSelectorsTestHW {
    ...
    public void cssSelectorsTest() {
        driver.get("file:D://TeachMeAQA//AQA21onl-testng//src//test//resources//Создать одностраничный Сайт на бесплатном шаблоне _ REG.RU.html");
        ...
        // Поиск по class name
        Assert.assertTrue(driver.findElement(By.cssSelector(".ds-header__menu-main-wrapper")).isDisplayed());

        //   аналог
        Assert.assertTrue(driver.findElement(By.className("ds-header__menu-main-wrapper")).isDisplayed());

        // Поиск по нескольким значения в аттрибуте class
        Assert.assertTrue(driver.findElement(By.cssSelector(".b-page__content-wrapper.b-page__content-wrapper_style_indent.b-websites__questions")).isDisplayed());
        ...
        Assert.assertTrue(driver.findElement(By.cssSelector("div.u-footer-content__sidebar")).isDisplayed());

        // Поиск всех элементов с тэгом h1 или p
        Assert.assertEquals(14, driver.findElements(By.cssSelector("h1, p")).size());

        // Поиск всех элементов с тэгом p у которых непосредственный родитель с тэгом div
        Assert.assertEquals(11, driver.findElements(By.cssSelector("div > p")).size());

        // Поиск всех элементов с тэгом p которые идут сразу за элементом с тэго ul
        Assert.assertEquals(0, driver.findElements(By.cssSelector("ul + p")).size());

        // Поиск всех элементов с тэгом div которые являются братьями элементу с тэгом p
        Assert.assertEquals(0, driver.findElements(By.cssSelector("p ~ div")).size());
        ...
        Assert.assertEquals(18, driver.findElements(By.cssSelector(":enabled")).size());
        ...
        Assert.assertEquals(137, driver.findElements(By.cssSelector(":empty")).size());

        // Поиск элемента с тэгом p и которым является первым дочерним элементом
        Assert.assertEquals(4, driver.findElements(By.cssSelector("p:first-child")).size());

        // Поиск элемента с тэгом p и которым является последним дочерним элементом
        Assert.assertEquals(0, driver.findElements(By.cssSelector("p:last-child")).size());

        // Поиск элемента с тэгом p и которым является n-ым дочерним элементом
        Assert.assertEquals(0, driver.findElements(By.cssSelector("p:nth-child(2)")).size());
    }
}
